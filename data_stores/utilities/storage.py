...
class FileStorage:
    ...
    def save_file(self, key: str, content: bytes, custom_metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Scrittura su filesystem via LocalFileStore
        ...
        rec = {
            ...
            "encoded_name": encoded_key,  # percorso fisico relativo encodato
            "size": file_path.stat().st_size,
            "modified_time": time.ctime(file_path.stat().st_mtime),
            "created_time": time.ctime(file_path.stat().st_ctime),
            "path": logical_key,  # ← MOSTRARE: percorso logico (NON encodato)
            "fs_path": str(file_path),  # ← USO INTERNO: percorso fisico sul disco
            "mime_type": mime_type,
    ...
    def get_file_metadata(self, key: str) -> dict:
        ...
        # Se non c'è un record, ricostruiscilo base
        if not rec:
            rec = {
                "name": logical_key,
                ...
                "modified_time": time.ctime(file_path.stat().st_mtime),
                "created_time": time.ctime(file_path.stat().st_ctime),
                "path": logical_key,  # ← MOSTRARE: percorso logico
                "fs_path": str(file_path),  # ← USO INTERNO: percorso fisico
                "mime_type": mime_type,
                "custom_metadata": {},
            }
        else:
            # Normalizza sempre i campi dinamici/derivati
            rec["name"] = logical_key
            rec["encoded_name"] = encoded_key
            rec["path"] = logical_key  # ← forza il percorso logico per la UI
            rec["fs_path"] = str(file_path)  # ← aggiorna il percorso fisico reale
            rec["mime_type"] = mime_type
            # (size/ctime/mtime li lasciamo così come sono in metadata.json)
